...
Original file is located at
    https://colab.research.google.com/drive/1nzha3_NzqgN1gV4unHEi_ePLW380d7xw
"""

#!/usr/bin/env python3
"""
Federated Learning on CIFAR10
- Saves model only if new accuracy is higher
- Resumes training from previous checkpoint if available
- Supports GPU, TensorBoard, malicious clients, mixup
"""
...
config = {
    ...
    "ckpt": "gm_best_100_Non_IID(Extreme).pt",
    "logdir": "runs/fl_training",
    "use_pretrained": True,
    "mixup": True,
    "mixup_alpha": 0.8,
    "malicious_ratio": 0.0,
...
malicious_ids = set(random.sample(range(num_clients),
                                  int(malicious_ratio * num_clients)))

print("Malicious clients:", malicious_ids)

for r in range(1, rounds + 1):

    selected = random.sample(range(num_clients),
                             max(1, int(client_frac * num_clients)))

    local_states, local_sizes = [], []

    for cid in selected:
        st, n = train_local(global_model, client_sets[cid], is_malicious=(cid in malicious_ids))
